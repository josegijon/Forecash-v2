        ...
        <>
            ...
            <tr className="group bg-card transition-colors hover:bg-muted/50 hidden xl:table-row">
                ...
                <td headers="col-amount" className="p-4">
                    <span className={`font-bold whitespace-nowrap ${isIncome ? "text-success" : "text-chart-line"}`}>
                        {currencySymbol}{fmt(amount)}
                    </span>
                </td>
                <td headers="col-action" className="p-4 rounded-r-xl">
                    <div className="flex items-center justify-end gap-2">
                        <button
                            onClick={onEdit}
                            aria-label={`Editar ${name}`}
                            className="text-slate-300 hover:text-primary transition-all cursor-pointer opacity-0 group-hover:opacity-100"
                        >
                            <Pencil size={16} />
                        </button>
                        <button
                            ...
                            className="text-slate-300 hover:text-destructive transition-all cursor-pointer opacity-0 group-hover:opacity-100"
                        >
                            <Trash2 size={18} />
                        </button>
                    </div>
                    ...
                    <div className={`flex items-center justify-between gap-2 bg-card rounded-2xl px-4 py-3 hover:bg-muted/50 transition-colors border-l-3 ${isIncome ? "border-success" : "border-chart-line"}`}>
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                            ...
                            <button
                                onClick={onEdit}
                                aria-label={`Editar ${name}`}
                                className="text-muted-foreground/40 hover:text-primary transition-all cursor-pointer p-1 rounded"
                            >
                                <Pencil size={15} />
                            </button>
                            <button
                                onClick={() => setConfirmOpen(true)}
                                aria-label={`Eliminar ${name}`}
                                className="text-muted-foreground/40 hover:text-destructive transition-all cursor-pointer p-1 rounded"
                            >
                                <Trash2 size={16} />
                            </button>
                        </div>
